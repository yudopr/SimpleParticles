...
class SimpleParticle {
	...
	#params = {
		...
		playState: {
			repeat:
				-1 /* -1 or 0 to repeat endlessly, 2 to repeat the particles 2 times, 3  to repeat the particles 2 times, and so on */,
		...
		start: {
			...
			rotation: { from: 0, to: 0 },
			color:
				"white" /* random return random color from #000000 to #ffffff, any other CSS acceptable colours */,
		},
		filter: {
			/* this effects is heavy resource, it will slows down your machine if use carelessly */
			blur: {
				active: true,
				value: 1,
	...
	#setupParticles(_containerID, _params) {
		...
		for (var i = 0; i < total; i++) {
			...
			if (container.childElementCount < total) {
				circ.setAttribute(
					"id",
					svgElement.getElementsByTagName("g")[0].getAttribute("id") + "-" + i
				);
		...
		if (!this.isParticlesPlaying()) {
			this.#timelines.forEach((element) => {
				element.play(
					Math.floor(
						Math.random() *
							(_params.playState.repeat < 0 ? 3105 : element.duration())
					) + 1
				);
			});
		}
	...
	addParticle(_containerID, _params) {
		...
		var group =
			svgElement.getElementsByTagName("g")[
				svgElement.getElementsByTagName("g").length - 1
			];
		for (var i = 0; i < total; i++) {
			var circ = document.createElementNS(SimpleParticle.#SVGNS, "circle");
			circ.setAttribute("id", group.getAttribute("id") + "-" + i);
			if (container.childElementCount < total) {
	...
	#animateParticle(elm, _tl) {
		var randomDuration = `random(${Math.round(
			elm.params.duration.from
		)}, ${Math.round(elm.params.duration.to)})`;
		/* Y-Axis animation */
		_tl.to(
			elm,
			{
		...
		_tl.to(
			elm,
			{
				...
				attr: {
					...
					cx: `+=${gsap.utils.random(elm.params.end.x.from, elm.params.end.x.to, 1)}`,
				},
			},
			0.2
		);
		...
		_tl.to(
			elm,
			{
				duration: randomDuration,
				attr: {
					fill: elm.params.colorOverlife
						? elm.params.end.color == "random"
							? this.#randomColor()
							: elm.params.end.color
						: elm.params.start.color,
				},
			},
			0.1
		);
	...
	#createParticleContainer(_parentElement, _params, _isNewParticle) {
		...
		if (_isNewParticle) {
			...
			svgElement.setAttribute(
				"viewBox",
				`0 0 ${_parentElement.offsetWidth} ${_parentElement.offsetHeight}`
			);
			...
			svgElement.appendChild(
				document.createElementNS(SimpleParticle.#SVGNS, "defs")
			); // for future update of using SVG filters
	...
	#createFilter(_target, _params) {
		...
		let gaussian = document.createElementNS(
			SimpleParticle.#SVGNS,
			"feGaussianBlur"
		);
	 ...
	 *
	 *
	 * @return {*} a random single number based on params
	 * @memberof SimpleParticle
	 */
	#randomInt(_min, _max) {
        return Math.floor(Math.random() * (Math.floor(_max) - Math.ceil(_min) + 1) + Math.ceil(_min));
	}
}